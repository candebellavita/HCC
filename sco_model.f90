...
!    CALL splev(t,M,s,k,X_interpol,KN_corr_interpol,Ntri,ier)
    ...
    !CALL sco_SIMPSON_complex(mesh_size,x2,auxiliar,dTesimps1)
    !auxiliar2 = x2 * auxiliar
    !CALL sco_SIMPSON_complex(mesh_size,x2,auxiliar2,dTesimps2)
    
    !dTe = 0
    !do i=1, mesh_size-2
    !    dTe = dTe + (k0(i) + k1(i) * dTesimps1 + k2(i) * dTesimps2) / (mesh_size - 2)
    !enddo
     
    ! fractional amplitude of the seed source temperature oscillation
    !auxiliar3 = auxiliar1 / Nescp
    !CALL sco_SIMPSON_complex(mesh_size,x2,auxiliar3,dTssimps)

    !dTs = p1 * dTssimps
    
    !write(*,*) 'dTe = ' , dTe , 'dTs=', dTs
    !dTe_mod, dTe_arg = np.abs(dTe), np.angle(dTe) # this is returned in the outputs
    !dTs_mod, dTs_arg = np.abs(dTs), np.angle(dTs) # this is returned in the outputs
